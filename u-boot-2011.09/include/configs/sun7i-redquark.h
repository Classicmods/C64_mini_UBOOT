...
/* Redquark config - we have dropped the 'env' and 'bootloader' partitionis. */
#define CONFIG_EXTRA_ENV_SETTINGS \
    "bootdelay=0\0" \
    "bootcmd=run setargs_nand boot_normalboot\0" \
    "console=ttyS0,115200\0" \
    "nand_root=/dev/nandb\0" \
    "init=/init\0" \
    "loglevel=8\0" \
    "setargs_nand=setenv bootargs console=${console} root=${nand_root}" \
    " init=${init} loglevel=${loglevel} partitions=${partitions} mem=256M quiet\0" \
    "boot_normal=sunxi_flash read 40007800 boot;boota 40007800\0" \
    "boot_recovery=sunxi_flash read 40007800 recovery;boota 40007800\0" \
    "boot_fastboot=fastboot\0" \
...
#define CONFIG_SUNXI_NO_LOGO    /* Don't attempt to display a logo */
